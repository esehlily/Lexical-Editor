import { $applyNodeReplacement, $caretFromPoint, $createLineBreakNode, $createParagraphNode, $createRangeSelection, $createTabNode, $findMatchingParent, $getAdjacentNode, $getCaretRange, $getChildCaret, $getEditor, $getNearestNodeFromDOMNode, $getRoot, $getSelection, $insertNodes, $isBlockElementNode, $isDecoratorNode, $isElementNode, $isNodeSelection, $isRangeSelection, $isRootNode, $isRootOrShadowRoot, $isTextNode, $isTextPointCaret, $normalizeSelection__EXPERIMENTAL as $normalizeSelection, $parseSerializedNode, $selectAll, $setSelection, ArtificialNode__DO_NOT_USE, CLICK_COMMAND, CONTROLLED_TEXT_INSERTION_COMMAND, COPY_COMMAND, CUT_COMMAND, DELETE_CHARACTER_COMMAND, DELETE_LINE_COMMAND, DELETE_WORD_COMMAND, DRAGOVER_COMMAND, DRAGSTART_COMMAND, DROP_COMMAND, ElementNode, FORMAT_ELEMENT_COMMAND, FORMAT_TEXT_COMMAND, INDENT_CONTENT_COMMAND, INSERT_LINE_BREAK_COMMAND, INSERT_PARAGRAPH_COMMAND, INSERT_TAB_COMMAND, KEY_ARROW_DOWN_COMMAND, KEY_ARROW_LEFT_COMMAND, KEY_ARROW_RIGHT_COMMAND, KEY_ARROW_UP_COMMAND, KEY_BACKSPACE_COMMAND, KEY_DELETE_COMMAND, KEY_ENTER_COMMAND, KEY_ESCAPE_COMMAND, KEY_SPACE_COMMAND, KEY_TAB_COMMAND, OUTDENT_CONTENT_COMMAND, PASTE_COMMAND, PASTE_TAG, REMOVE_TEXT_COMMAND, SELECTION_INSERT_CLIPBOARD_NODES_COMMAND, SELECT_ALL_COMMAND, addClassNamesToElement, createCommand, defineExtension, getDOMSelection, getRegisteredNode, isBlockDomNode, isDOMDocumentNode, isDOMNode, isDocumentFragment, isHTMLElement, isInlineDomNode, isSelectionCapturedInDecoratorInput, isSelectionWithinEditor, mergeRegister, safeCast, setNodeIndentFromDOM } from "./lexical.js";
import { a as $getNearestBlockElementAncestorOrThrow, c as $addNodeStyle, d as $shouldOverrideDefaultCharacterSelection, f as $sliceSelectedTextNodeContent, l as $isParentRTL, o as $handleIndentAndOutdent, r as namedSignals, s as objectKlassEquals, t as E, u as $moveCharacter } from "./LexicalExtension.dev-DJIxxTFy.js";
//#region node_modules/@lexical/html/LexicalHtml.dev.mjs
/**
...
function $generateNodesFromDOM(editor, dom) {
	...
	for (const element of elements) if (!IGNORE_TAGS.has(element.nodeName)) {
		...
		if (lexicalNode !== null) lexicalNodes = lexicalNodes.concat(lexicalNode);
	}
	$unwrapArtificialNodes(allArtificialNodes);
	return lexicalNodes;
}
function $generateHtmlFromNodes(editor, selection) {
	if (typeof document === "undefined" || typeof window === "undefined" && typeof global.window === "undefined") throw new Error("To use $generateHtmlFromNodes in headless mode please initialize a headless browser implementation such as JSDom before calling this function.");
...
function $appendNodesToHTML(editor, currentNode, parentElement, selection = null) {
	...
	let exportOutput;
	if (registeredNode && registeredNode.exportDOM !== void 0) exportOutput = registeredNode.exportDOM(editor, target);
	else exportOutput = target.exportDOM(editor);
	...
	for (let i = 0; i < children.length; i++) {
		...
		if (!shouldInclude && $isElementNode(currentNode) && shouldIncludeChild && currentNode.extractWithChild(childNode, selection, "html")) shouldInclude = true;
	}
	if (shouldInclude && !shouldExclude) {
		if (isHTMLElement(element) || isDocumentFragment(element)) element.append(fragment);
		parentElement.append(element);
		if (after) {
			const newElement = after.call(target, element);
			if (newElement) if (isDocumentFragment(element)) element.replaceChildren(newElement);
			else element.replaceWith(newElement);
		}
	} else parentElement.append(fragment);
...
function $createNodesFromDOM(node, editor, allArtificialNodes, hasBlockAncestorLexicalNode, forChildMap = /* @__PURE__ */ new Map(), parentLexicalNode) {
	...
	if (isBlockDomNode(node)) if (!hasBlockAncestorLexicalNodeForChildren) childLexicalNodes = wrapContinuousInlines(node, childLexicalNodes, $createParagraphNode);
	...
	if (currentLexicalNode == null) {
		if (childLexicalNodes.length > 0) lexicalNodes = lexicalNodes.concat(childLexicalNodes);
		else if (isBlockDomNode(node) && isDomNodeBetweenTwoInlineNodes(node)) lexicalNodes = lexicalNodes.concat($createLineBreakNode());
...
function $unwrapArtificialNodes(allArtificialNodes) {
	for (const node of allArtificialNodes) if (node.getNextSibling() instanceof ArtificialNode__DO_NOT_USE) node.insertAfter($createLineBreakNode());
...
//#endregion
//#region node_modules/@lexical/clipboard/LexicalClipboard.dev.mjs
/**
* Copyright (c) Meta Platforms, Inc. and affiliates.
*
* This source code is licensed under the MIT license found in the
...
function $getClipboardDataFromSelection(selection = $getSelection()) {
	const clipboardData = { "text/plain": selection ? selection.getTextContent() : "" };
	if (selection) {
		const editor = $getEditor();
		for (const [mimeType, $editorFn] of clipboardDataFunctions) {
			const v = $editorFn(editor, selection);
			if (v !== null) clipboardData[mimeType] = v;
		}
	}
...
//#endregion
//#region node_modules/@lexical/dragon/LexicalDragon.dev.mjs
/**
* Copyright (c) Meta Platforms, Inc. and affiliates.
*
* This source code is licensed under the MIT license found in the
...
function registerDragonSupport(editor) {
	...
	const handler = (event) => {
		...
		const rootElement = editor.getRootElement();
		if (document.activeElement !== rootElement) return;
...
//#endregion
//#region node_modules/@lexical/rich-text/LexicalRichText.dev.mjs
/**
* Copyright (c) Meta Platforms, Inc. and affiliates.
*
* This source code is licensed under the MIT license found in the
...
var HeadingNode = class HeadingNode extends ElementNode {
	...
	insertNewAfter(selection, restoreSelection = true) {
		...
		const newElement = !lastDesc || selection && selection.anchor.key === lastDesc.getKey() && anchorOffet === lastDesc.getTextContentSize() || !selection ? $createParagraphNode() : $createHeadingNode(this.getTag());
		const direction = this.getDirection();
...
function isGoogleDocsTitle(domNode) {
	if (domNode.nodeName.toLowerCase() === "span") return domNode.style.fontSize === "26pt";
	return false;
}
...
function onPasteForRichText(event, editor) {
	...
	editor.update(() => {
		...
		if (clipboardData != null && selection !== null) $insertDataTransferForRichText(clipboardData, selection, editor);
	}, { tag: PASTE_TAG });
}
...
function $isTargetWithinDecorator(target) {
	return $isDecoratorNode($getNearestNodeFromDOMNode(target));
}
...
function registerRichText(editor) {
	return mergeRegister(editor.registerCommand(CLICK_COMMAND, (payload) => {
		...
		return false;
	}, 0), editor.registerCommand(DELETE_WORD_COMMAND, (isBackward) => {
		const selection = $getSelection();
		if (!$isRangeSelection(selection)) return false;
		selection.deleteWord(isBackward);
		return true;
	}, 0), editor.registerCommand(DELETE_LINE_COMMAND, (isBackward) => {
		const selection = $getSelection();
		if (!$isRangeSelection(selection)) return false;
		selection.deleteLine(isBackward);
		return true;
	}, 0), editor.registerCommand(CONTROLLED_TEXT_INSERTION_COMMAND, (eventOrText) => {
		const selection = $getSelection();
		if (typeof eventOrText === "string") {
			if (selection !== null) selection.insertText(eventOrText);
		} else {
			if (selection === null) return false;
			const dataTransfer = eventOrText.dataTransfer;
			if (dataTransfer != null) $insertDataTransferForRichText(dataTransfer, selection, editor);
		...
		return true;
	}, 0), editor.registerCommand(REMOVE_TEXT_COMMAND, () => {
		const selection = $getSelection();
		if (!$isRangeSelection(selection)) return false;
		selection.removeText();
		return true;
	}, 0), editor.registerCommand(FORMAT_TEXT_COMMAND, (format) => {
		const selection = $getSelection();
		if (!$isRangeSelection(selection)) return false;
		selection.formatText(format);
		return true;
	}, 0), editor.registerCommand(FORMAT_ELEMENT_COMMAND, (format) => {
		const selection = $getSelection();
		...
		for (const node of nodes) {
			...
			if (element !== null) element.setFormat(format);
		}
		return true;
	}, 0), editor.registerCommand(INSERT_LINE_BREAK_COMMAND, (selectStart) => {
		const selection = $getSelection();
		if (!$isRangeSelection(selection)) return false;
		selection.insertLineBreak(selectStart);
		return true;
	}, 0), editor.registerCommand(INSERT_PARAGRAPH_COMMAND, () => {
		const selection = $getSelection();
		if (!$isRangeSelection(selection)) return false;
		selection.insertParagraph();
		return true;
	}, 0), editor.registerCommand(INSERT_TAB_COMMAND, () => {
	...
	}, 0), editor.registerCommand(OUTDENT_CONTENT_COMMAND, () => {
		return $handleIndentAndOutdent((block) => {
			const indent = block.getIndent();
			if (indent > 0) block.setIndent(Math.max(0, indent - 1));
		});
		...
		return false;
	}, 0), editor.registerCommand(KEY_ARROW_DOWN_COMMAND, (event) => {
		const selection = $getSelection();
		if ($isNodeSelection(selection)) {
			const nodes = selection.getNodes();
			if (nodes.length > 0) {
		...
		return false;
	}, 0), editor.registerCommand(KEY_ARROW_LEFT_COMMAND, (event) => {
		const selection = $getSelection();
		if ($isNodeSelection(selection)) {
			const nodes = selection.getNodes();
			if (nodes.length > 0) {
		...
		return false;
	}, 0), editor.registerCommand(KEY_ARROW_RIGHT_COMMAND, (event) => {
		const selection = $getSelection();
		if ($isNodeSelection(selection)) {
			const nodes = selection.getNodes();
			if (nodes.length > 0) {
		...
		return false;
	}, 0), editor.registerCommand(KEY_BACKSPACE_COMMAND, (event) => {
		if ($isTargetWithinDecorator(event.target)) return false;
		const selection = $getSelection();
		if ($isRangeSelection(selection)) {
			...
			if (IS_IOS && navigator.language === "ko-KR") return false;
		} else if (!$isNodeSelection(selection)) return false;
		event.preventDefault();
		return editor.dispatchCommand(DELETE_CHARACTER_COMMAND, true);
	}, 0), editor.registerCommand(KEY_DELETE_COMMAND, (event) => {
		if ($isTargetWithinDecorator(event.target)) return false;
		const selection = $getSelection();
		if (!($isRangeSelection(selection) || $isNodeSelection(selection))) return false;
		event.preventDefault();
		return editor.dispatchCommand(DELETE_CHARACTER_COMMAND, false);
	}, 0), editor.registerCommand(KEY_ENTER_COMMAND, (event) => {
		const selection = $getSelection();
		if (!$isRangeSelection(selection)) return false;
	...
	}, 0), editor.registerCommand(KEY_ESCAPE_COMMAND, () => {
		if (!$isRangeSelection($getSelection())) return false;
		editor.blur();
		return true;
	}, 0), editor.registerCommand(DROP_COMMAND, (event) => {
		...
		return false;
	}, 0), editor.registerCommand(DRAGSTART_COMMAND, (event) => {
		const [isFileTransfer] = eventFiles(event);
		const selection = $getSelection();
		if (isFileTransfer && !$isRangeSelection(selection)) return false;
		return true;
	}, 0), editor.registerCommand(DRAGOVER_COMMAND, (event) => {
		const [isFileTransfer] = eventFiles(event);
		const selection = $getSelection();
		if (isFileTransfer && !$isRangeSelection(selection)) return false;
		const x = event.clientX;
		...
		return true;
	}, 0), editor.registerCommand(SELECT_ALL_COMMAND, () => {
		$selectAll();
		return true;
	}, 0), editor.registerCommand(COPY_COMMAND, (event) => {
		copyToClipboard(editor, objectKlassEquals(event, ClipboardEvent) ? event : null);
		return true;
	}, 0), editor.registerCommand(CUT_COMMAND, (event) => {
		onCutForRichText(event, editor);
		return true;
	}, 0), editor.registerCommand(PASTE_COMMAND, (event) => {
		...
		return false;
	}, 0), editor.registerCommand(KEY_SPACE_COMMAND, (_) => {
		const selection = $getSelection();
		if ($isRangeSelection(selection)) $resetCapitalization(selection);
		return false;
	}, 0), editor.registerCommand(KEY_TAB_COMMAND, (_) => {
		const selection = $getSelection();
		if ($isRangeSelection(selection)) $resetCapitalization(selection);
		return false;
	}, 0));
